...
nematode_mods <- nematode_w %>%
  ...
  pivot_longer(cols = b:nli, names_to = "exp_var", values_to = "exp_val") %>%
  # Creating groups of resp_var and exp_var
  group_by(resp_var,exp_var) %>%
  nest() %>%
  # Applying linear model to each element
  mutate(lm = map(data, ~lm(resp_val ~ exp_val, data = .x))) %>% #.x is the same as before data

  # Extracting R2 for each element  
  mutate(r2 = map(lm, ~summary(.x)$r.squared))
...
nematode_mods_sel_plots <- nematode_mods_sel %>%
  ...
  mutate(splot = map2(data, r2, ~ggplot(data = .x, aes(x = exp_val, y= resp_val))+
                        geom_point()+
  ...
  mutate(splot_better = pmap(list(.df= data,
                             ...
                             function(.df, .r2,.rv,.ev)
                               ggplot(data = .df, aes(x = exp_val, y= resp_val))+
                                geom_point()+
                                  geom_smooth(method ="lm")+
                                  labs(title = paste("R2:",.r2), x = .ev, y = .rv)
                             ))
